...
-- MAGIC %md
-- MAGIC #Criando o governando objetos de dados com Unity Catalog
-- MAGIC 
-- MAGIC ###Objetivos:
...
-- MAGIC Qualquer pessoa com experiência em SQL provavelmente estará familiarizada com o namespace tradicional de dois níveis para endereças tabelas ou views em um schema da seguinte maneira:
-- MAGIC 
-- MAGIC     SELECT * FROM myschema.mytable;
-- MAGIC     
-- MAGIC O Unity Catalog apresenta o conceito de **catalog** na hierarquia. Como um contêiner para schemas, o catálogo fornece uma nova forma para orgazinações segregar seus dados. Pode haver quantos catálogos você quiser, que por sua vez pode conter quantos svhemas você quiser (o conceito de schema não é alterado pelo Unity Catalog. Os schemas contém objetos de dados como tabelasm views e UDFs)
-- MAGIC 
...
-- MAGIC Os desenvolvedores de SQL provavelmente tmbém estarão familiarizados com a instrução **`USE`** para selecionar um schema padrão, encurtando assim as consultas por não precisar especificá-lo o tempo todo. Para estender essa conveniência ao lidar com o nível extra no namespace, o Unity Catalog aumenta a linguagem com duas instruções adicionais: 
-- MAGIC 
...
-- MAGIC Vamos configurar as tabelas e views. Para este exemplo, usaremnos dados fictícios para criar e preencher uma tabela Silver Managed com dados sintéticos de frequência cardíaca do paciente e uma view Gold que calcula a média diária dos dados de frequência cardíaca por paciente.
-- MAGIC 
...
-- MAGIC %md
-- MAGIC Consultar a tabela acima funciona como esperado, pois somos os proprietários dos dados. Ou seja, temos a propriedade do objeto de dados que estamos consultando. Consultar a view também funciona porque somos os proprietários da view e da tabela à qual ela faz referência. Portanto, nenhuma permissão em npivel de objeto é necessária para acessar esses recursos.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC ###Conceder acesso a objeto de dados (Grant)
-- MAGIC O Unity Catalog emprega um modelo de permissão explícita por padrão. Nenhuma permissão é implícita ou herdada. Potanto, para acessar quaiquer objetos de dados, os usuários precisarão da permissão **USAGE** em todos os elementos contidos, ou seja, o schema e o catálogo que o contém.
-- MAGIC 
...
-- MAGIC 
-- MAGIC   A célula a seguir gera uma instrução de consulta totalmente qualificada que especifica todos os três níveis para a exibição, pois a executaremos em um ambiente que não possui variáveis ou um catálogo e svhema padrão configurados. Execute a consulta gerada abaixo na sessão SQL do Databricks. Como todas as concessões apropriadas estão disponpiveis para os analistas acessarem a view, a saída deve se parecer com o que vimos anteriormente ao consultar a view gold.
...
-- MAGIC O Unity Catalog é capaz de gerenciar UDFs dentro de schemas. O cpodigo a seguir configura uma função simples que mascara todos, exceto os dois últimos caracteres de uma string, e então a testa. Mais uma vez, somos os proprietários dos dados, portanto, nenhuma concessão é necessária.
...
-- MAGIC Cole a instrução de consulta totalmente qualificada gerada abaixo em uma nova consulta para executar esta função na sessão SQL do Databricks. Como todas as concessões apropriedas estão disponíveis para os analistas acessarem a função, a saída deve se parecer com o que acabalos de ver acima.
...
-- MAGIC 
-- MAGIC As views dinâmicas fornecem a capacidade de fazer um controle de acesso refinado de colunas e linhas em uma tabela, condicional ao principal que executa a consulta. As views dinâmicas são uma extenção das views padrão que nos permitem fazer as coisas como:
...
-- MAGIC 
-- MAGIC O controle de acesso com views dinâmicas é obtido por meio do usu de funções dentro da definição da view. Essas funções incluem:
...
-- MAGIC Suponha que queremos que os analistas possam ver as tendências de dados agergados da view *gold*, mas não queremos divulgar o usuário. Vamos redefinir a visualização para redigir as colunas *mrn* e *name* usando **`is_account_group_member()`**.
-- MAGIC 
-- MAGIC Observação: este é um exemplo de treinamento simples que não vecessariamente se alinha com as práticas recomendadas gerais. Para um sistema de produção, uma abordagem mais segura seria redigir os valores de coluna para todos os usuários que não são membros de um grupo específico.
...
-- MAGIC %md
-- MAGIC Para nós, isso gera uma saída não filtrada, pois somos membros do grupo *analysts*. Agora, revisire a sessão SQL do Databricks e execute novamente a consulta na view *gold* como um analista. Veremos que os valores das colunas *mrn* e *name* foram editados.

-- COMMAND ----------

-- MAGIC %md
-- MAGIC ###Restringir linhas
-- MAGIC Agora vamos supor que queremos uma view que, em vez de agregar e editar as colunas, ela simplesmente filtre as linhas da origem. Vamos aplivar a mesma função **`is_account_group_member()`** para criar uma view que passe apenas por linhas cuji *device_id* seja menor que 30. A filtragem de linha é feita aplicando uma condicional como uma cláusula **WHERE**.
...
-- MAGIC Um último caso de uso para views dinâmicas é o mascaramento de dados ou obscurecimento parcial dos dados. No primeiro exemplo, redogimos as colunas inteiramente. O mascaramento é semelhante em princípio, exceto que estamos exibindo alguns dos dados em vez de substituí-los totalmente. E para este exemplo simples, aproveitaremos a UDF *mask()* que criamos anteriormente para mascarar a coluna *mrn* para nossos analistas, embora o SQL forneça uma biblioteca bastante abrangente de funções de manipulação de dados integradas que podem ser aproveitadas para mascarar dados de várias maneiras diferentes. É uma boa prática aproveitá-los quando puder.
...
-- MAGIC Vamos explorar algumas instruções SQL para examinar nosos objetos de dados e permissões. Vamos começar fazendo um balanço dos objetos que temos no schema *examples*.
...
-- MAGIC %md
-- MAGIC Revisite a sessão SQL do Databricks e execute novamente a consulta na view *gold* como analista. Observe qu isso ainda funciona como antes.
-- MAGIC 
-- MAGIC Lembre-se de que a view está efetivamente sendo executada como um proprietário, que também possui a função e a tabela de origem. Assim como o analista não exigia acesso direto à tabela que está sendo consultada, pois o proprietário da view é proprietário da tabela, a função continua a funcinar pelo mesmo motivo.
-- MAGIC 
