...
# MAGIC - Usar parâmetros no Pipeline DLT
...
# MAGIC
# MAGIC No momento em que este notebook foi escrito, o tempo de execução atual do Databricks não inclui o módulo dlt, portanto, tentar executar qualquer comando DLT em um notebook falhará.
# MAGIC
...
# MAGIC
# MAGIC No centro desse disign, o decorador **`@dlt.table`** é adicionado a qualquer função Python que retorna um Spark DataFrame. (Observação: isso inclui Koalas DataFrame, mas eles não serão abordados neste curso)
# MAGIC
# MAGIC Se você está acostumado a trabalhar com Spark e/ou streaming estruturado, você reconhecerá a maior parte da sintaxe usada no DTL. A grande diferença é que você nunca verá nenhum método ou opção para gravar Dataframe, pois essa lógica é tratada pelo DTL.
# MAGIC
