...
-- MAGIC
-- MAGIC No notebook anterior, revisamos algumas funcionalidades básicas da sintaxe DLT durante o processamento de dados do armazenamento de objetos na nuvem por meio de uma série de consultas para validar e enriquecer os registros em cada etapa. Este notebook também segue a arquitetura medalhão, mas apresenta uma série de novos conceitos.
...
-- MAGIC
-- MAGIC O código abaixo também fornece um comentário simples e adiciona campos para a hora da ingestão de dados e o nome de cada registro.
...
-- MAGIC
-- MAGIC As views ainda podem ser usadas para impor a qualidade dos dados, e as métricas das views serão coletadas e relatadas como seriam para as tabelas.
-- MAGIC
...
-- MAGIC
-- MAGIC Essencilamente, podemos pensar no escopo da referência do banco de dados pela palavra chava **`LIVE`** para estar no nível do DLT pipeline, em vez do notebook individual.
-- MAGIC
