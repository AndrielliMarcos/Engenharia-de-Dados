...
-- MAGIC Este notebook fornece ums revisão prática de algumas das funcionalidades básicas do Databricks Data Science & Engineering Workspace.
-- MAGIC 
...
-- MAGIC %md
-- MAGIC ###Anexar um cluster
-- MAGIC Executar células em um notebook requer recursos computacionais, como os que são fornecidos pelos clusters. A primeira vez que você executar uma célula no notebook, será solicitadi anexa um cluster se ainda não tiver anexado.
-- MAGIC 
...
-- MAGIC 
-- MAGIC Este notebook declara a variável `my_name`, definida como `None`, e então cria um Dstaframe chamado `example_df`.
-- MAGIC 
...
-- MAGIC As vezes é útil limpar todas as variáveis definidas no notebook e iniciar do começo. Isso pode ser útil quando você que testar células isoladas ou você quer simplesmente reiniciar o estado de execução.
-- MAGIC 
