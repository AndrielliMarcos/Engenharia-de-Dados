...
-- MAGIC Depois de extrair os dados da fonte de dados externas, carregue os dados no Lakehouse para garantir que todos os benefícios do Databricks possam ser totalmente aproveitados.
-- MAGIC
...
-- MAGIC %md
-- MAGIC As instruções CTAS inferem as informações do schema automaticamente do resultado da consulta e não oferecem suporte à declaração manual do esquema. 
-- MAGIC
...
-- MAGIC
-- MAGIC As instruções CTAS também não oferecem suporte à especificação de opções de arquivos adicionais.
-- MAGIC
...
-- MAGIC
-- MAGIC Na lição anterior, nós mostramos como fazer isso registrando uma tabela externa. Aqui, desenvolveremos ligeiramente essa sintaxe para especificar as opções para uma temp view  e, em seguida, usaremos essa temp view como a origem de uma instrução CTAS para registrar com êxito a tabela Delta
...
-- MAGIC
-- MAGIC A instrução a seguir cria uma nova tabela contendo um subconjunto de colunas da tabela **`sale`**.
-- MAGIC
...
-- MAGIC %md
-- MAGIC Como **`date`** é uma coluna gerada, se escrevermos em **`purchase_dates`** sem fornecer valores para a coluna **`date`**, o Delta Lake os calculará automaticamente.
-- MAGIC
...
-- MAGIC
-- MAGIC A seguir, mostramos a evolução de uma instrução CTAS para incluir várias configurações e metadados adicionais.
-- MAGIC
-- MAGIC Nossa cláusula **`SELECT`** utiliza dois comandos integrados do Spark SQL úteis para ingestão de arquivos:
-- MAGIC * **`current_timestamp()`** registra o timestamp de quando a lógica é executada
...
-- MAGIC %md
-- MAGIC Em ambos os casos, as modificações de dados aplicadas à versão clonada da tabela serão rastreadas e armazenadas separadamente da origem. A clonagem é uma ótima maneira de configurar tabelas para testar o código SQL ainda em desenvolvimento.
