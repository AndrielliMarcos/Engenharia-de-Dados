...
-- MAGIC 
-- MAGIC Este notebook demostra o uso de Delta Live Table (DLT) pata processar dados brutos de arquivos JSON que chegam ao armazenamento de objetos na nuvem por meio de uma série de tabelas para direcionar cargas de trabalho analíticas no Lakehouse. Aqui, vamos demostrar a arquitetura medalhão, no qual os dados são transformados e enriquecidos incrementalmente à medida que fluem por um pipeline. Este notebook foca na sictaxe SQL do DLT, em vez dessa arquitetura, mas uma breve visão geral:
...
-- MAGIC - A tabela gold contém dados agragados que direcionam insights de negócios e dashboards
-- MAGIC 
-- MAGIC ###Objetivos:
-- MAGIC - Declara Delta Live Tables
-- MAGIC - Ingerir dados com Auto Loader
-- MAGIC - Usar parâmetros nos pipelines DLT
-- MAGIC - Forças qualidade nos dados com constraints
...
-- MAGIC * **Live tables** são views materializadas para o Lakehouse. Elas irão retornar o resultado atuais de qualquer consulta a cada atualização
-- MAGIC * **Streaming live tables** são projetadas para processamente de dados incremental, quase em tempo real
-- MAGIC 
...
-- MAGIC 
-- MAGIC Para ambos os tipos de tabelas, DLT adota a abordagem de uma instrução CTAS(create tabel as select) levemente modificada. O engenheiros só precisam se preocupar em escrever consultas para transformar seus dados e DLT cuida do resto.
-- MAGIC 
...
-- MAGIC - o formato do dados de origem, que neste caso é JSON
-- MAGIC - uma lista separada por vírgulas de tamenho arbitrário de opções do leitor. Neste caso, definimos **`cloudFiles.inferColumnTypes`** para **`true`**
-- MAGIC 
...
-- MAGIC DLT permite que usuários declarem facilmente tabelas a partir de resultados de qualquer transformação Spark padrão. DLT aproveita recursos usados  em outros lugares no Spar SQL para documentar conjuntos de dados, enquanto adiciona novas funcionalidades para verificar a aqualidade dos dados.
-- MAGIC 
...
-- MAGIC 
-- MAGIC Cada constraint pode ter várias condições e várias constraints podem ser definidas para uma única tabela. Além de falhar na atualização, a violação de uma constraint também pode descartar registros automaticamente ou apenas registrar o número de violações emquanto ainda processa esses registros inválidos.
-- MAGIC 
...
-- MAGIC O campo **`TBLPROPERTIES`** pode ser usado para passar qualquer número de pares de chave/valor para marcação personalizada de dados. Aqui, definimos o valor prata para a qualidade da chave.
-- MAGIC 
