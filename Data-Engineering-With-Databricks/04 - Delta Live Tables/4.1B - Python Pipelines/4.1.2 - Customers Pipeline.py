...
# MAGIC O código abaixo demonstra o uso da option **`name`** para declarar a tabela DLT. A option permite desenvolvedores especificar o nome da tabela resultante separada da definição da função que cria o DataFrame a partir do qual a tabela é definida.
# MAGIC
...
# MAGIC
# MAGIC ##### **`valid_address`**
# MAGIC Essa constraint verifica se o campo **`operation`** é **`DELETE`**. Se não for, ele verifica valores nulos em qualquer um dos 4 campos que compõem um endereço. Como não há instruções adicionais sobre o que fazer com registros inválidos, as linhas com violação serão registradas nas métricas, mas não descartadas.
# MAGIC
# MAGIC ##### **`valid_email`**
...
# MAGIC - ordena automaticamente os registros atrasados usando a chave de sequenciamento fornecida pelo usuário
...
# MAGIC
# MAGIC Esse padrão garante que, se alguma atualização chegar fora da ordem, os resultados downstream possam ser recalculados adequadamente para refletir as atualizações. Ele também garante que, quando os registros são excluídos de uma tabela de origem, esses valores não sejam mais refletidos nas tabelas posteriormente no pipeline.
# MAGIC
# MAGIC Abaixo, definimos uma consulta agregada simples para criar uma tabela dinâmica a partir dos dados da tabela **`customers_silver`**.
...
# MAGIC
# MAGIC As views em DLT diferem das tabelas persistentes e também podem herdar a execução de streaming da função que decoram.
# MAGIC
...
# MAGIC
# MAGIC Ao contrário das views usadas em outros lugares no Databricks, as Views DLT  não são persistidas no metastores, o que significa que elas só podem ser referenciadas de dentro do pipeline DLT do qual fazem parte (Este é um escopo semelhante ao DataFrame em notebooks databricks).
# MAGIC
...
# MAGIC
# MAGIC Essencilamente, podemos pensar no escopo da referência do banco de dados pela palavra chave **`LIVE`** para estar no nível do DLT pipeline, em vez do notebook individual.
# MAGIC
...
# MAGIC    * Use o seletor de arquivos para selecionar este notebook, em seguida clique em **Select**
