...
-- MAGIC A célula abaixo condensa todas as transações de lição anterior em uma única célula. (Exceto o comando para dropar a tabela)
-- MAGIC
-- MAGIC
...
-- MAGIC Acabamos de ver muitos arquivos de dados para o que obviamente é uma tabela muito pequena.
-- MAGIC **`DESCRIBE DETAIL`** nos permite ver alguns outros detalhes sobre nossa Delta table, incluindo o número de arquivos.
...
-- MAGIC
-- MAGIC Essas consultas podem ser executadas especificando a versão inteira ou um timestamp.
-- MAGIC
